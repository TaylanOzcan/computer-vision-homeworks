...
%
        ...
        img_dx = filter2(F_X, currentImage);
        img_dy = filter2(F_Y, currentImage);
        
        for i = 1:size(img_dx, 1)
            for j = 1:size(img_dx, 2)
                grad_theta{scale}(i,j) = atan2(img_dy(i,j), img_dx(i,j));
                grad_mag{scale}(i,j) = sqrt( (img_dx(i,j)*img_dx(i,j)) + (img_dy(i,j)*img_dy(i,j)) );
                grad_theta{scale}(i,j) = grad_theta{scale}(i,j) - pi/2;    
            end
        end
...
%
    ...
    [val, index] = max(histogram);
    direction = (index - 1) / num_bins * (2 * pi);
...
%        
    for i = 1:length(gradient_angles)
        for j = 1:(length(angles)-1)
            if gradient_angles(i) >= angles(j) && gradient_angles(i) < angles(j+1)
...
%
    ...
    for i = 1:grid_size
        for j = 1:grid_size
            grid_i_end = i*pixelsPerCell; grid_i_start = (grid_i_end - pixelsPerCell + 1);
            grid_j_end = j*pixelsPerCell; grid_j_start = (grid_j_end - pixelsPerCell + 1);
            grid_mag = patch_mag(grid_i_start : grid_i_end, grid_j_start : grid_j_end);
            grid_theta = patch_theta(grid_i_start : grid_i_end, grid_j_start : grid_j_end);
            histogram = ComputeGradientHistogram(num_bins, grid_mag, grid_theta);
