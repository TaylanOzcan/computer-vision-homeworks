...
%% Initializations
    ...
    for i = 1 : maxIter
        ...
        err = ComputeError(H, p1(non_seed_group(:, 1), :), p2(non_seed_group(:, 2),:), match);
        ...
        %                     YOUR CODE HERE:                             %
        %
        for j = 1:length(non_seed_group)
            if (err(j) < maxInlierError)
                inliers = [inliers, non_seed_group(j, :)] ;
            end
        end
        %                                                                 %
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        if( number_of_inliers > goodFitThresh )
            ...
            %                     YOUR CODE HERE:                         %
            %  
            H_new = ComputeAffineMatrix(p1(inliers(:, 1), :), p2(inliers(:, 2), :));
            err_new = ComputeError(H_new, p1(inliers(:, 1), :), p2(inliers(:, 2),:), match);
...
%
    ...
    P1 = [pt1';ones(1,N)];
    P1_transformed = H*P1;
    P2_computed = [(P1_transformed(1, 1:size(pt1,1)))', (P1_transformed(2, 1:size(pt1,1)))'];
    dists = sqrt(sum((P2_computed-pt2).^2,2));
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
